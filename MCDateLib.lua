...
if not MCDateLib or MCDateLib.Version < MCDateLib_cVersion then
	...
	function MCDateLib:GetServerDateTime()
		...
		if not vMonth or vMonth <= 0 then
			self:DebugMessage("GetServerDateTime: CalendarGetDate() not ready")
			self:DebugStack()
			return
		end
		
		return self:ConvertMDYToDate(vMonth, vDay, vYear), self:GetServerTime()
	end
	...
	function MCDateLib:CalculateTimeZoneOffset()
		...
		local vServerTimeZoneOffset = self:RoundTimeOffsetToNearest30(vLocalDateTime - vServerDateTime)
	...
	function MCDateLib:GetDaysToDate(pMonth, pDay, pYear)
		...
		if not self.cDaysToMonth[pMonth] then
			self:DebugMessage("GetDaysToDate: Invalid month %s", tostring(pMonth))
			self:DebugStack()
			return 0
		end
	...
	MCDebugLib:InstallDebugger("MCDateLib", MCDateLib, {r=0.5, g=1, b=0.5})
end
